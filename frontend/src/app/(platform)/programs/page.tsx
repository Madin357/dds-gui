          ...
          <tbody>
            ...
            : programs.map((p) => (
              <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50">
                <td className="px-4 py-3"><div><p className="font-medium text-slate-700">{t(`programNames.${p.name}` as any) || p.name}</p><p className="text-xs text-slate-400">{p.code} • {p.department ? (t(`faculties.${p.department}` as any) || p.department) : p.level}</p></div></td>
